use crate::mlkem::{MLKEM512_K, MLKEM512_ETA1, MLKEM512_DU, MLKEM512_DV, MLKEM512_ENCAPS_K, MLKEM512_DECAPS_K};
use crate::mlkem::keys;
use crate::mlkem::keyCipher;
use crate::mlkem::keys_init;
use crate::mlkem::keyCipher_init;
use crate::utils::concat_arrays;
use crate::hashes::H;
use crate::hashes::G;
use crate::mlkem::kpke;

/// MLKEM-512 key generation
/// Returns a struct containing the encapsulation key and decapsulation key
/// # Returns
/// * `keys` - struct containing ek and dk
pub fn mlkem_key_gen_512_impl() -> keys{
    // print!("Running mlkem_key_gen_512_impl\n");
    // internal needs two random 32byte seeds
    // let k = mlkem_key_gen_512_internal(@get_seed512(), @get_z512());
    let d = @get_seed512();
    let z = @get_z512();
    if(d.len() != 32_usize || z.len() != 32_usize){
        panic!("Seeds must be 32 bytes long");
    }

    // print!("Running mlkem_key_gen_512_internal\n")
    let kpkeKeys = kpke::kpke_keygen(d, MLKEM512_K, MLKEM512_ETA1, MLKEM512_DU, MLKEM512_DV);
    let ekpke = kpkeKeys.ek;
    let dkpke = kpkeKeys.dk;
    
    // hex is sha3-256(ekpke), H function
    let hek = H(ekpke.clone());
    let dkpke_ekpke = concat_arrays(@dkpke, @ekpke);
    let tmp2 = concat_arrays(@dkpke_ekpke, @hek);

    let mut k : keys = keys_init();
    k.ek = ekpke;
    k.dk = concat_arrays(@tmp2, z);
    k.ek_len = MLKEM512_ENCAPS_K.try_into().unwrap();
    k.dk_len = MLKEM512_DECAPS_K.try_into().unwrap();
    // print!("Encapsulation key should be length: {}\n", MLKEM512_ENCAPS_K);
    // print!("Decapsulation key should be length: {}\n", MLKEM512_DECAPS_K);
    // print!("K has public key length: {} and secret key length: {}\n", k.ek.len(), k.dk.len());
    // print!("Struct says ek {} and dk {} bytes\n", k.ek_len, k.dk_len);
    // k
    // print!("K has public key length: {} and secret key length: {}\n", k.ek.len(), k.dk.len());
    k
}

/// MLKEM-512 encapsulation
/// # Arguments
/// * `ek` - encapsulation key
/// * `m` - message to be encapsulated - hardcoded for testing
/// # Returns
/// * `keyCipher` - struct containing the shared key and ciphertext
pub fn mlkem_encaps_512_impl( ek : @Array<u8> ) -> keyCipher{
    // print!("Running mlkem_encaps_512_impl\n");
    // internal needs one random 32byte for the message
    let m = @get_message512();
    if(m.len() != 32_usize){
        panic!("Message must be 32 bytes long");
    }
    // print!("Message obtained\n");

    // should print this thing for testing
    // ek last 10 bytes: 3e aa de da 51 19 62 a3 6e b0 
    // hek last 10 bytes: f0 51 ff 23 5b 8d fb b7 3a 4d 
    // r last 10 bytes: 6d c1 f5 fd f8 f 2d a0 1c a7 
    
    // the following code is split into segments for clarity
    let ek_array = ek.clone();
    let hek = H(ek_array);
    let m_hek = concat_arrays(m, @hek);
    
    let ek_span = ek.clone().span();
    let ek_slice = ek_span.slice(ek_span.len() - 10, 10);
    print!("ek last 10 bytes: ");
    for byte in ek_slice{
        print!(" {:x}", *byte);
    }
    print!("\n");

    let hek_span = hek.span();
    let hek_slice = hek_span.slice(hek_span.len() - 10, 10);
    print!("hek last 10 bytes: ");
    for byte in hek_slice{
        print!(" {:x}", *byte);
    }
    print!("\n");

    // derive shared key K and randomness r
    let (K, r ) = G(concat_arrays(m, @H(ek.clone())));

    // print last ten bytes of r
    let r_span = r.span();
    let r_slice = r_span.slice(r_span.len() - 10, 10);
    print!("r last 10 bytes: ");
    for byte in r_slice{
        print!(" {:x}", *byte);
    }
    print!("\n");

    // panic!("Stopping after printing last 10 bytes of r");

    let c = kpke::kpke_encrypt(ek, m, @r, MLKEM512_K, MLKEM512_ETA1, MLKEM512_DU, MLKEM512_DV);
    let mut kc : keyCipher = keyCipher_init();
    kc.key = K;
    kc.c = c;
    // print!("Ciphertext should be length: {}\n", MLKEM512_CIPHER);
    print!("KeyCipher has key length: {} and ciphertext length: {}\n", kc.key.len(), kc.c.len());
    print!("Struct says k_len {} and c_len {} bytes\n", kc.k_len, kc.c_len);
    kc
}

// pub fn mlkem_key_gen_512_impl() -> keys{
//     print!("Running mlkem_key_gen_512_impl\n");
//     // internal needs two random 32byte seeds
//     let k = mlkem_key_gen_512_internal(@get_seed512(), @get_z512());
//     print!("K has public key length: {} and secret key length: {}\n", k.ek.len(), k.dk.len());
//     k
// }


// pub fn mlkem_key_gen_768_impl() -> keys{
//     mlkem_internal::mlkem_key_gen_768_internal()
// }
// pub fn mlkem_key_gen_1024_impl() -> keys{
//     mlkem_internal::mlkem_key_gen_1024_internal()
// }

// pub fn mlkem_key_gen_512_internal( d : @Array<u8>, z : @Array<u8> ) -> keys{
//     if(d.len() != 32_usize || z.len() != 32_usize){
//         panic!("Seeds must be 32 bytes long");
//     }

//     // print!("Running mlkem_key_gen_512_internal\n");
//     let mut kpkeKeys : keys = keys_init();
//     let kpkeKeys = kpke::kpke_keygen(d, MLKEM512_K, MLKEM512_ETA1, MLKEM512_DU, MLKEM512_DV);
//     let ekpke = kpkeKeys.ek;
//     let dkpke = kpkeKeys.dk;
    
//     // hex is sha3-256(ekpke), H function
//     let hek = H(ekpke.clone());
//     let dkpke_ekpke = concat_arrays(@dkpke, @ekpke);
//     let tmp2 = concat_arrays(@dkpke_ekpke, @hek);

//     let mut k : keys = keys_init();
//     k.ek = ekpke;
//     k.dk = concat_arrays(@tmp2, z);
//     k.ek_len = MLKEM512_ENCAPS_K.try_into().unwrap();
//     k.dk_len = MLKEM512_DECAPS_K.try_into().unwrap();
//     // print!("Encapsulation key should be length: {}\n", MLKEM512_ENCAPS_K);
//     // print!("Decapsulation key should be length: {}\n", MLKEM512_DECAPS_K);
//     // print!("K has public key length: {} and secret key length: {}\n", k.ek.len(), k.dk.len());
//     // print!("Struct says ek {} and dk {} bytes\n", k.ek_len, k.dk_len);
//     k
// }



// values for testing purposes
fn get_seed512() -> Array<u8>{
//     let mut seed512 : [u8; 32] = [ 0xe1, 0xe3, 0x20, 0x68, 0x75, 0xe6, 0x7d, 0x7e, 0x81, 0x35,
// 0x37, 0x74, 0xfe, 0x90, 0x25, 0x03, 0x5b, 0x9b, 0x41, 0xa4, 0xa9, 0xf6,
// 0xec, 0x00, 0xb9, 0x1c, 0x60, 0x04, 0x42, 0xfd, 0x71, 0x7d ];
    let mut arr = ArrayTrait::new();
    arr.append(0xe1);
    arr.append(0xe3);
    arr.append(0x20);
    arr.append(0x68);
    arr.append(0x75);
    arr.append(0xe6);
    arr.append(0x7d);
    arr.append(0x7e);
    arr.append(0x81);
    arr.append(0x35);
    arr.append(0x37);
    arr.append(0x74);
    arr.append(0xfe);
    arr.append(0x90);
    arr.append(0x25);
    arr.append(0x03);
    arr.append(0x5b);
    arr.append(0x9b);
    arr.append(0x41);
    arr.append(0xa4);
    arr.append(0xa9);
    arr.append(0xf6);
    arr.append(0xec);
    arr.append(0x00);
    arr.append(0xb9);
    arr.append(0x1c);
    arr.append(0x60);
    arr.append(0x04);
    arr.append(0x42);
    arr.append(0xfd);
    arr.append(0x71);
    arr.append(0x7d);
    arr
}

fn get_z512() -> Array<u8>{
    // z512 = { 0xc6, 0xf5, 0x78, 0x5a, 0x6f, 0x2b, 0x42, 0xe8, 0x43, 0x22,
    // 0x8b, 0xe5, 0x3e, 0xb7, 0x68, 0xd6, 0x4c, 0x6f, 0x9d, 0x43, 0x55, 0xae, 0x95, 0xf0,
    // 0x83, 0xe5, 0x1e, 0xd5, 0x7c, 0x43, 0x73, 0x10 };
    let mut arr = ArrayTrait::new();
    arr.append(0xc6);
    arr.append(0xf5);
    arr.append(0x78);
    arr.append(0x5a);
    arr.append(0x6f);
    arr.append(0x2b);
    arr.append(0x42);
    arr.append(0xe8);
    arr.append(0x43);
    arr.append(0x22);
    arr.append(0x8b);
    arr.append(0xe5);
    arr.append(0x3e);
    arr.append(0xb7);
    arr.append(0x68);
    arr.append(0xd6);
    arr.append(0x4c);
    arr.append(0x6f);
    arr.append(0x9d);
    arr.append(0x43);
    arr.append(0x55);
    arr.append(0xae);
    arr.append(0x95);
    arr.append(0xf0);
    arr.append(0x83);
    arr.append(0xe5);
    arr.append(0x1e);
    arr.append(0xd5);
    arr.append(0x7c);
    arr.append(0x43);
    arr.append(0x73);
    arr.append(0x10);    
    arr
}

fn get_message512() -> Array<u8>{
    // message512 = { 0xa7, 0x41, 0xec, 0x20, 0x02, 0xbe, 0x6f, 0x4f, 0xa7,
    //     0x60, 0x37, 0xb7, 0xf0, 0x64, 0x4f, 0x83, 0x3f, 0xa8, 0x23, 0xe6, 0x30, 0x40, 0x1a,
    //     0x39, 0xd3, 0x24, 0x0c, 0x6e, 0x82, 0xa4, 0x30, 0xbb };
    let mut arr = ArrayTrait::new();
    arr.append(0xa7);
    arr.append(0x41);
    arr.append(0xec);
    arr.append(0x20);
    arr.append(0x02);
    arr.append(0xbe);
    arr.append(0x6f);
    arr.append(0x4f);
    arr.append(0xa7);
    arr.append(0x60);
    arr.append(0x37);
    arr.append(0xb7);
    arr.append(0xf0);
    arr.append(0x64);
    arr.append(0x4f);
    arr.append(0x83);
    arr.append(0x3f);
    arr.append(0xa8);
    arr.append(0x23);
    arr.append(0xe6);
    arr.append(0x30);
    arr.append(0x40);
    arr.append(0x1a);
    arr.append(0x39);
    arr.append(0xd3);
    arr.append(0x24);
    arr.append(0x0c);
    arr.append(0x6e);
    arr.append(0x82);
    arr.append(0xa4);
    arr.append(0x30);
    arr.append(0xbb);    
    arr
}

pub fn get_ek() -> Array<u8>{
    let mut arr = ArrayTrait::new();
    arr.append(0xf2);
arr.append(0x9c);
arr.append(0x86);
arr.append(0x6c);
arr.append(0x36);
arr.append(0x1d);
arr.append(0x91);
arr.append(0x03);
arr.append(0x41);
arr.append(0xf2);
arr.append(0x96);
arr.append(0xc6);
arr.append(0x4b);
arr.append(0x46);
arr.append(0xc2);
arr.append(0xa2);
arr.append(0xe3);
arr.append(0x0b);
arr.append(0x15);
arr.append(0x35);
arr.append(0xa5);
arr.append(0xc0);
arr.append(0x60);
arr.append(0x25);
arr.append(0x93);
arr.append(0x41);
arr.append(0x5d);
arr.append(0x15);
arr.append(0x6b);
arr.append(0x43);
arr.append(0x03);
arr.append(0x6b);
arr.append(0x21);
arr.append(0xb1);
arr.append(0x4b);
arr.append(0xa3);
arr.append(0xa0);
arr.append(0xc7);
arr.append(0x2e);
arr.append(0x84);
arr.append(0x8c);
arr.append(0x5e);
arr.append(0xe0);
arr.append(0x34);
arr.append(0x66);
arr.append(0xca);
arr.append(0xb9);
arr.append(0x77);
arr.append(0x21);
arr.append(0xd8);
arr.append(0x3d);
arr.append(0xef);
arr.append(0xda);
arr.append(0xb1);
arr.append(0xf6);
arr.append(0x70);
arr.append(0x89);
arr.append(0x71);
arr.append(0xd3);
arr.append(0xc4);
arr.append(0x58);
arr.append(0x14);
arr.append(0x41);
arr.append(0xce);
arr.append(0x39);
arr.append(0x7f);
arr.append(0xe7);
arr.append(0xbb);
arr.append(0xc7);
arr.append(0xa0);
arr.append(0x8a);
arr.append(0xcf);
arr.append(0xdc);
arr.append(0xc4);
arr.append(0x17);
arr.append(0xc5);
arr.append(0xf5);
arr.append(0x6d);
arr.append(0xfd);
arr.append(0xd4);
arr.append(0x42);
arr.append(0x3e);
arr.append(0x30);
arr.append(0x6f);
arr.append(0x1f);
arr.append(0x69);
arr.append(0xa8);
arr.append(0x6e);
arr.append(0x5b);
arr.append(0x5b);
arr.append(0xa3);
arr.append(0xe0);
arr.append(0x31);
arr.append(0xbf);
arr.append(0x92);
arr.append(0xa1);
arr.append(0x67);
arr.append(0x02);
arr.append(0x86);
arr.append(0x1a);
arr.append(0x51);
arr.append(0xa2);
arr.append(0xfe);
arr.append(0xb9);
arr.append(0x74);
arr.append(0xf4);
arr.append(0x84);
arr.append(0x48);
arr.append(0x12);
arr.append(0xb8);
arr.append(0x30);
arr.append(0x2d);
arr.append(0xc8);
arr.append(0x30);
arr.append(0x26);
arr.append(0xea);
arr.append(0x56);
arr.append(0x6a);
arr.append(0x1c);
arr.append(0x11);
arr.append(0x0c);
arr.append(0xd2);
arr.append(0xc4);
arr.append(0xaf);
arr.append(0x48);
arr.append(0xbe);
arr.append(0xdd);
arr.append(0x74);
arr.append(0x2f);
arr.append(0x1c);
arr.append(0x02);
arr.append(0xcd);
arr.append(0x3f);
arr.append(0xca);
arr.append(0x0d);
arr.append(0xfb);
arr.append(0x83);
arr.append(0x31);
arr.append(0xf4);
arr.append(0x14);
arr.append(0xb1);
arr.append(0x15);
arr.append(0x84);
arr.append(0x9b);
arr.append(0xcb);
arr.append(0x05);
arr.append(0xa2);
arr.append(0xdf);
arr.append(0x61);
arr.append(0x5b);
arr.append(0x49);
arr.append(0xa9);
arr.append(0x0e);
arr.append(0xc0);
arr.append(0xa4);
arr.append(0x33);
arr.append(0xe8);
arr.append(0x78);
arr.append(0x7e);
arr.append(0x18);
arr.append(0x51);
arr.append(0x42);
arr.append(0xaa);
arr.append(0x40);
arr.append(0x60);
arr.append(0x03);
arr.append(0xd3);
arr.append(0x54);
arr.append(0x09);
arr.append(0xec);
arr.append(0x84);
arr.append(0x2a);
arr.append(0xf3);
arr.append(0x04);
arr.append(0x45);
arr.append(0x9b);
arr.append(0x40);
arr.append(0xff);
arr.append(0xfa);
arr.append(0x0d);
arr.append(0x84);
arr.append(0x34);
arr.append(0x01);
arr.append(0x56);
arr.append(0xb5);
arr.append(0x76);
arr.append(0x7e);
arr.append(0xe0);
arr.append(0xca);
arr.append(0x6e);
arr.append(0x28);
arr.append(0x50);
arr.append(0xec);
arr.append(0x8c);
arr.append(0xca);
arr.append(0xed);
arr.append(0x31);
arr.append(0x7e);
arr.append(0xcf);
arr.append(0x71);
arr.append(0x1e);
arr.append(0x27);
arr.append(0xd5);
arr.append(0x9b);
arr.append(0x3c);
arr.append(0x98);
arr.append(0x55);
arr.append(0x55);
arr.append(0xe4);
arr.append(0x4c);
arr.append(0x93);
arr.append(0x59);
arr.append(0x88);
arr.append(0x09);
arr.append(0x29);
arr.append(0x81);
arr.append(0xda);
arr.append(0x95);
arr.append(0xb4);
arr.append(0x25);
arr.append(0x73);
arr.append(0xb9);
arr.append(0xbc);
arr.append(0x38);
arr.append(0x12);
arr.append(0x2e);
arr.append(0xd4);
arr.append(0x4a);
arr.append(0x80);
arr.append(0x05);
arr.append(0x10);
arr.append(0x75);
arr.append(0x64);
arr.append(0x29);
arr.append(0xf1);
arr.append(0xb0);
arr.append(0x64);
arr.append(0x06);
arr.append(0x85);
arr.append(0xb0);
arr.append(0x48);
arr.append(0x59);
arr.append(0x78);
arr.append(0x42);
arr.append(0xcb);
arr.append(0xb9);
arr.append(0x3d);
arr.append(0xc6);
arr.append(0xb2);
arr.append(0xf9);
arr.append(0x14);
arr.append(0x4d);
arr.append(0xcf);
arr.append(0x41);
arr.append(0xa6);
arr.append(0x24);
arr.append(0xeb);
arr.append(0x90);
arr.append(0x20);
arr.append(0x2a);
arr.append(0x30);
arr.append(0x38);
arr.append(0x14);
arr.append(0x10);
arr.append(0xf4);
arr.append(0xcb);
arr.append(0xaa);
arr.append(0xcc);
arr.append(0xf6);
arr.append(0x1e);
arr.append(0x48);
arr.append(0x55);
arr.append(0x24);
arr.append(0xee);
arr.append(0x59);
arr.append(0x3a);
arr.append(0xd7);
arr.append(0xd5);
arr.append(0x97);
arr.append(0x4f);
arr.append(0x77);
arr.append(0x4a);
arr.append(0xe5);
arr.append(0x9b);
arr.append(0xa2);
arr.append(0x0c);
arr.append(0x7c);
arr.append(0x6e);
arr.append(0xb0);
arr.append(0x00);
arr.append(0x9b);
arr.append(0x39);
arr.append(0x96);
arr.append(0x2d);
arr.append(0xc0);
arr.append(0xf8);
arr.append(0x28);
arr.append(0xe8);
arr.append(0x53);
arr.append(0x8e);
arr.append(0xf5);
arr.append(0x83);
arr.append(0xb4);
arr.append(0xdc);
arr.append(0x40);
arr.append(0xc0);
arr.append(0x95);
arr.append(0x55);
arr.append(0xa3);
arr.append(0x96);
arr.append(0x8a);
arr.append(0xb2);
arr.append(0x57);
arr.append(0xd4);
arr.append(0x07);
arr.append(0xcc);
arr.append(0x4a);
arr.append(0xb8);
arr.append(0x01);
arr.append(0xc7);
arr.append(0x52);
arr.append(0x1b);
arr.append(0xf7);
arr.append(0x56);
arr.append(0x4b);
arr.append(0x16);
arr.append(0x3f);
arr.append(0xab);
arr.append(0x92);
arr.append(0x7b);
arr.append(0x47);
arr.append(0x03);
arr.append(0x22);
arr.append(0x5b);
arr.append(0x1b);
arr.append(0x49);
arr.append(0x7a);
arr.append(0x09);
arr.append(0x6b);
arr.append(0x4e);
arr.append(0xb9);
arr.append(0x84);
arr.append(0xdf);
arr.append(0xd3);
arr.append(0x51);
arr.append(0x2e);
arr.append(0xcb);
arr.append(0x2e);
arr.append(0xae);
arr.append(0xa4);
arr.append(0x6f);
arr.append(0xb5);
arr.append(0x3c);
arr.append(0x3b);
arr.append(0x38);
arr.append(0x40);
arr.append(0xaf);
arr.append(0x3c);
arr.append(0x4a);
arr.append(0xaf);
arr.append(0x78);
arr.append(0xb1);
arr.append(0x46);
arr.append(0x1e);
arr.append(0x05);
arr.append(0x64);
arr.append(0x29);
arr.append(0x0c);
arr.append(0x0a);
arr.append(0x8f);
arr.append(0x93);
arr.append(0xa1);
arr.append(0xe1);
arr.append(0xb1);
arr.append(0xc5);
arr.append(0x0c);
arr.append(0xdb);
arr.append(0x5f);
arr.append(0xd5);
arr.append(0x69);
arr.append(0x70);
arr.append(0x04);
arr.append(0x61);
arr.append(0x03);
arr.append(0x60);
arr.append(0x0a);
arr.append(0x1c);
arr.append(0xc1);
arr.append(0x09);
arr.append(0xb5);
arr.append(0x8c);
arr.append(0xc3);
arr.append(0x2e);
arr.append(0x52);
arr.append(0xc7);
arr.append(0xa1);
arr.append(0x68);
arr.append(0xe1);
arr.append(0x71);
arr.append(0x07);
arr.append(0xa9);
arr.append(0xc7);
arr.append(0x7c);
arr.append(0xc4);
arr.append(0x48);
arr.append(0xb0);
arr.append(0x1a);
arr.append(0x5a);
arr.append(0x7d);
arr.append(0xc9);
arr.append(0x06);
arr.append(0x03);
arr.append(0xd7);
arr.append(0x08);
arr.append(0xc6);
arr.append(0xd7);
arr.append(0x17);
arr.append(0xc3);
arr.append(0x16);
arr.append(0x5e);
arr.append(0x77);
arr.append(0x2c);
arr.append(0x74);
arr.append(0x76);
arr.append(0x38);
arr.append(0x21);
arr.append(0xf3);
arr.append(0x59);
arr.append(0x34);
arr.append(0xa6);
arr.append(0x13);
arr.append(0xb0);
arr.append(0x29);
arr.append(0x72);
arr.append(0x46);
arr.append(0x37);
arr.append(0x00);
arr.append(0x20);
arr.append(0xeb);
arr.append(0x11);
arr.append(0x36);
arr.append(0x45);
arr.append(0xd7);
arr.append(0x55);
arr.append(0x93);
arr.append(0xc5);
arr.append(0x13);
arr.append(0x00);
arr.append(0x72);
arr.append(0xac);
arr.append(0xc0);
arr.append(0x48);
arr.append(0x31);
arr.append(0x18);
arr.append(0x06);
arr.append(0x7b);
arr.append(0xfb);
arr.append(0x80);
arr.append(0x93);
arr.append(0xb5);
arr.append(0x18);
arr.append(0x06);
arr.append(0x63);
arr.append(0x18);
arr.append(0xc3);
arr.append(0xf9);
arr.append(0x36);
arr.append(0xbd);
arr.append(0x29);
arr.append(0xe8);
arr.append(0xac);
arr.append(0xba);
arr.append(0x83);
arr.append(0x18);
arr.append(0xc6);
arr.append(0x8a);
arr.append(0x3c);
arr.append(0x47);
arr.append(0xa1);
arr.append(0xbe);
arr.append(0x71);
arr.append(0x3c);
arr.append(0x5b);
arr.append(0x5a);
arr.append(0x3c);
arr.append(0x3d);
arr.append(0xf8);
arr.append(0x25);
arr.append(0x14);
arr.append(0xa5);
arr.append(0xdb);
arr.append(0x7f);
arr.append(0x70);
arr.append(0x15);
arr.append(0x0b);
arr.append(0x44);
arr.append(0xb3);
arr.append(0x92);
arr.append(0x7a);
arr.append(0xa5);
arr.append(0xc7);
arr.append(0xb5);
arr.append(0x50);
arr.append(0x7f);
arr.append(0x8e);
arr.append(0x69);
arr.append(0x3e);
arr.append(0xed);
arr.append(0x04);
arr.append(0x36);
arr.append(0x93);
arr.append(0x40);
arr.append(0x14);
arr.append(0x1b);
arr.append(0x1c);
arr.append(0x75);
arr.append(0x06);
arr.append(0x9b);
arr.append(0x5f);
arr.append(0x3a);
arr.append(0xbb);
arr.append(0x01);
arr.append(0x68);
arr.append(0x33);
arr.append(0xb9);
arr.append(0x36);
arr.append(0xa1);
arr.append(0xad);
arr.append(0xf7);
arr.append(0xcc);
arr.append(0xbe);
arr.append(0xdf);
arr.append(0xe7);
arr.append(0x75);
arr.append(0xf0);
arr.append(0x49);
arr.append(0x4a);
arr.append(0xcb);
arr.append(0x64);
arr.append(0x04);
arr.append(0x30);
arr.append(0x7a);
arr.append(0x78);
arr.append(0x7e);
arr.append(0x19);
arr.append(0xb2);
arr.append(0xfc);
arr.append(0xba);
arr.append(0x2a);
arr.append(0xa8);
arr.append(0xb3);
arr.append(0xcf);
arr.append(0x94);
arr.append(0xea);
arr.append(0x6a);
arr.append(0xa4);
arr.append(0x30);
arr.append(0x3c);
arr.append(0xfe);
arr.append(0x48);
arr.append(0x21);
arr.append(0xfc);
arr.append(0x9a);
arr.append(0x62);
arr.append(0xee);
arr.append(0x5a);
arr.append(0xcb);
arr.append(0xca);
arr.append(0xa4);
arr.append(0x25);
arr.append(0x56);
arr.append(0xd0);
arr.append(0x46);
arr.append(0xe4);
arr.append(0xab);
arr.append(0x2e);
arr.append(0x65);
arr.append(0xe5);
arr.append(0x7e);
arr.append(0x4d);
arr.append(0xcc);
arr.append(0x87);
arr.append(0x42);
arr.append(0x76);
arr.append(0xc8);
arr.append(0x9f);
arr.append(0x98);
arr.append(0x0a);
arr.append(0xc1);
arr.append(0xa5);
arr.append(0x2f);
arr.append(0xde);
arr.append(0x54);
arr.append(0x90);
arr.append(0x64);
arr.append(0xb7);
arr.append(0xab);
arr.append(0xea);
arr.append(0xb3);
arr.append(0x0e);
arr.append(0x81);
arr.append(0xd5);
arr.append(0x40);
arr.append(0xaf);
arr.append(0x39);
arr.append(0x3a);
arr.append(0x51);
arr.append(0xbb);
arr.append(0x91);
arr.append(0x7d);
arr.append(0x87);
arr.append(0x09);
arr.append(0x8a);
arr.append(0xe2);
arr.append(0x38);
arr.append(0x94);
arr.append(0x60);
arr.append(0x33);
arr.append(0xf3);
arr.append(0xbb);
arr.append(0x31);
arr.append(0xd8);
arr.append(0x2b);
arr.append(0x41);
arr.append(0x8e);
arr.append(0xa8);
arr.append(0x63);
arr.append(0xb9);
arr.append(0xfc);
arr.append(0x20);
arr.append(0x08);
arr.append(0x61);
arr.append(0x3e);
arr.append(0x47);
arr.append(0x51);
arr.append(0xae);
arr.append(0xd8);
arr.append(0xc9);
arr.append(0xaa);
arr.append(0xa4);
arr.append(0x18);
arr.append(0x40);
arr.append(0x57);
arr.append(0x91);
arr.append(0x16);
arr.append(0xb5);
arr.append(0x71);
arr.append(0xad);
arr.append(0x6d);
arr.append(0xf0);
arr.append(0xcf);
arr.append(0xc1);
arr.append(0x18);
arr.append(0x5b);
arr.append(0x17);
arr.append(0x61);
arr.append(0x20);
arr.append(0x08);
arr.append(0x90);
arr.append(0x0d);
arr.append(0x10);
arr.append(0x4a);
arr.append(0x83);
arr.append(0xb6);
arr.append(0x7a);
arr.append(0x36);
arr.append(0xac);
arr.append(0xd8);
arr.append(0xc7);
arr.append(0x83);
arr.append(0xc5);
arr.append(0xb1);
arr.append(0xc2);
arr.append(0x10);
arr.append(0x26);
arr.append(0xf0);
arr.append(0x3b);
arr.append(0x4a);
arr.append(0xb5);
arr.append(0x7b);
arr.append(0x23);
arr.append(0xbd);
arr.append(0x16);
arr.append(0x0a);
arr.append(0x10);
arr.append(0x39);
arr.append(0x20);
arr.append(0x72);
arr.append(0x9b);
arr.append(0x13);
arr.append(0x33);
arr.append(0x43);
arr.append(0x95);
arr.append(0xa5);
arr.append(0x00);
arr.append(0xa2);
arr.append(0xa5);
arr.append(0xa1);
arr.append(0x26);
arr.append(0xbd);
arr.append(0x66);
arr.append(0xc7);
arr.append(0x5c);
arr.append(0x5c);
arr.append(0x2a);
arr.append(0x04);
arr.append(0x6a);
arr.append(0x97);
arr.append(0xeb);
arr.append(0xb8);
arr.append(0x56);
arr.append(0x5c);
arr.append(0xbb);
arr.append(0x65);
arr.append(0xfc);
arr.append(0x6a);
arr.append(0x3e);
arr.append(0x1c);
arr.append(0x11);
arr.append(0xa9);
arr.append(0x90);
arr.append(0xe2);
arr.append(0x0a);
arr.append(0x9c);
arr.append(0x88);
arr.append(0xc9);
arr.append(0x6a);
arr.append(0xa3);
arr.append(0x92);
arr.append(0x49);
arr.append(0x61);
arr.append(0x15);
arr.append(0x9e);
arr.append(0xf2);
arr.append(0x20);
arr.append(0x82);
arr.append(0xe4);
arr.append(0x5c);
arr.append(0x86);
arr.append(0xa9);
arr.append(0xbb);
arr.append(0x80);
arr.append(0xd7);
arr.append(0xb8);
arr.append(0x93);
arr.append(0x6c);
arr.append(0xc9);
arr.append(0xee);
arr.append(0xc5);
arr.append(0x93);
arr.append(0xe5);
arr.append(0xe4);
arr.append(0x52);
arr.append(0x3d);
arr.append(0xdc);
arr.append(0x54);
arr.append(0x38);
arr.append(0xc8);
arr.append(0x19);
arr.append(0x55);
arr.append(0x13);
arr.append(0x28);
arr.append(0x55);
arr.append(0x79);
arr.append(0x82);
arr.append(0xcf);
arr.append(0x67);
arr.append(0x84);
arr.append(0xb1);
arr.append(0x72);
arr.append(0x0e);
arr.append(0x4e);
arr.append(0xd5);
arr.append(0xac);
arr.append(0x0a);
arr.append(0xdd);
arr.append(0x45);
arr.append(0x7f);
arr.append(0x57);
arr.append(0x3a);
arr.append(0x04);
arr.append(0x14);
arr.append(0x65);
arr.append(0xbc);
arr.append(0xbd);
arr.append(0x7c);
arr.append(0xa4);
arr.append(0xe1);
arr.append(0xd7);
arr.append(0xd5);
arr.append(0x3e);
arr.append(0xaa);
arr.append(0xde);
arr.append(0xda);
arr.append(0x51);
arr.append(0x19);
arr.append(0x62);
arr.append(0xa3);
arr.append(0x6e);
arr.append(0xb0);
arr
}

pub fn get_dk() -> Array<u8>{
    let mut arr = ArrayTrait::new();
    arr.append(0x2d);
arr.append(0x17);
arr.append(0xc4);
arr.append(0xba);
arr.append(0x26);
arr.append(0x2e);
arr.append(0xde);
arr.append(0xc0);
arr.append(0x0f);
arr.append(0x00);
arr.append(0xf2);
arr.append(0x24);
arr.append(0x76);
arr.append(0x92);
arr.append(0xb8);
arr.append(0x7b);
arr.append(0xc2);
arr.append(0x56);
arr.append(0x4f);
arr.append(0xfc);
arr.append(0x79);
arr.append(0x10);
arr.append(0xf1);
arr.append(0x13);
arr.append(0xfb);
arr.append(0x8c);
arr.append(0x7d);
arr.append(0x8b);
arr.append(0x1c);
arr.append(0x39);
arr.append(0x81);
arr.append(0xac);
arr.append(0x95);
arr.append(0xd8);
arr.append(0xa8);
arr.append(0x48);
arr.append(0x87);
arr.append(0x81);
arr.append(0xbe);
arr.append(0xda);
arr.append(0xb1);
arr.append(0x91);
arr.append(0xe6);
arr.append(0x91);
arr.append(0x70);
arr.append(0x15);
arr.append(0x1c);
arr.append(0x2c);
arr.append(0xe9);
arr.append(0x42);
arr.append(0x19);
arr.append(0x12);
arr.append(0x76);
arr.append(0x69);
arr.append(0x1e);
arr.append(0xeb);
arr.append(0x39);
arr.append(0x5c);
arr.append(0xc2);
arr.append(0x9a);
arr.append(0xfd);
arr.append(0xe6);
arr.append(0x09);
arr.append(0x15);
arr.append(0x91);
arr.append(0xcc);
arr.append(0xef);
arr.append(0x03);
arr.append(0x8a);
arr.append(0xe9);
arr.append(0x23);
arr.append(0x5a);
arr.append(0x20);
arr.append(0x15);
arr.append(0x10);
arr.append(0xee);
arr.append(0xf0);
arr.append(0x14);
arr.append(0x55);
arr.append(0x46);
arr.append(0x26);
arr.append(0x01);
arr.append(0x0c);
arr.append(0x95);
arr.append(0x74);
arr.append(0xe7);
arr.append(0xcc);
arr.append(0xfa);
arr.append(0x56);
arr.append(0x57);
arr.append(0xd0);
arr.append(0x99);
arr.append(0x94);
arr.append(0x45);
arr.append(0x66);
arr.append(0x40);
arr.append(0x45);
arr.append(0xfb);
arr.append(0x20);
arr.append(0xa2);
arr.append(0x2a);
arr.append(0x5b);
arr.append(0x31);
arr.append(0x68);
arr.append(0x26);
arr.append(0x8b);
arr.append(0x12);
arr.append(0xab);
arr.append(0xfb);
arr.append(0x99);
arr.append(0x33);
arr.append(0xe2);
arr.append(0x48);
arr.append(0x6e);
arr.append(0x45);
arr.append(0xb8);
arr.append(0x4b);
arr.append(0x8d);
arr.append(0x39);
arr.append(0x73);
arr.append(0x78);
arr.append(0x5c);
arr.append(0x8e);
arr.append(0x15);
arr.append(0x89);
arr.append(0x15);
arr.append(0xfb);
arr.append(0x49);
arr.append(0x94);
arr.append(0x93);
arr.append(0xe0);
arr.append(0x19);
arr.append(0x5c);
arr.append(0xf7);
arr.append(0x04);
arr.append(0x43);
arr.append(0xbc);
arr.append(0xc4);
arr.append(0x51);
arr.append(0x7b);
arr.append(0x7c);
arr.append(0xc3);
arr.append(0x23);
arr.append(0xb0);
arr.append(0x05);
arr.append(0x1b);
arr.append(0xa3);
arr.append(0x1a);
arr.append(0xe4);
arr.append(0x4b);
arr.append(0xdb);
arr.append(0x41);
arr.append(0x5e);
arr.append(0x63);
arr.append(0xc6);
arr.append(0x6f);
arr.append(0xa0);
arr.append(0xf1);
arr.append(0x43);
arr.append(0x34);
arr.append(0xf7);
arr.append(0x5e);
arr.append(0x37);
arr.append(0x64);
arr.append(0x11);
arr.append(0xb9);
arr.append(0x1b);
arr.append(0xb7);
arr.append(0x29);
arr.append(0x6a);
arr.append(0x18);
arr.append(0xf3);
arr.append(0x23);
arr.append(0xb6);
arr.append(0xef);
arr.append(0x37);
arr.append(0xbe);
arr.append(0xe5);
arr.append(0xc8);
arr.append(0xb8);
arr.append(0x4e);
arr.append(0xcb);
arr.append(0xb6);
arr.append(0x5f);
arr.append(0x9c);
arr.append(0x00);
arr.append(0xbd);
arr.append(0x50);
arr.append(0x4b);
arr.append(0x31);
arr.append(0x6a);
arr.append(0x9a);
arr.append(0x6a);
arr.append(0x7c);
arr.append(0x60);
arr.append(0x91);
arr.append(0xd2);
arr.append(0x8f);
arr.append(0xe6);
arr.append(0x5c);
arr.append(0x34);
arr.append(0x36);
arr.append(0xb5);
arr.append(0xc9);
arr.append(0xc0);
arr.append(0xf8);
arr.append(0x9a);
arr.append(0x47);
arr.append(0x10);
arr.append(0x47);
arr.append(0x67);
arr.append(0xf7);
arr.append(0xb0);
arr.append(0xf7);
arr.append(0x9a);
arr.append(0x8f);
arr.append(0xc2);
arr.append(0x90);
arr.append(0x1d);
arr.append(0x49);
arr.append(0xe3);
arr.append(0x16);
arr.append(0x11);
arr.append(0xb2);
arr.append(0xb1);
arr.append(0x8c);
arr.append(0x52);
arr.append(0xb5);
arr.append(0x10);
arr.append(0xac);
arr.append(0x05);
arr.append(0x4e);
arr.append(0x92);
arr.append(0x62);
arr.append(0x29);
arr.append(0xf5);
arr.append(0x32);
arr.append(0xd2);
arr.append(0x18);
arr.append(0x76);
arr.append(0x57);
arr.append(0x24);
arr.append(0x48);
arr.append(0x36);
arr.append(0x25);
arr.append(0x4c);
arr.append(0x19);
arr.append(0x30);
arr.append(0x14);
arr.append(0xfd);
arr.append(0xe7);
arr.append(0x66);
arr.append(0xe8);
arr.append(0xd2);
arr.append(0x90);
arr.append(0x38);
arr.append(0xe0);
arr.append(0xa3);
arr.append(0x88);
arr.append(0x77);
arr.append(0x4c);
arr.append(0xb5);
arr.append(0x53);
arr.append(0x35);
arr.append(0x43);
arr.append(0x97);
arr.append(0x32);
arr.append(0xbb);
arr.append(0x03);
arr.append(0x23);
arr.append(0x99);
arr.append(0xc3);
arr.append(0x23);
arr.append(0x01);
arr.append(0x05);
arr.append(0x2a);
arr.append(0x8f);
arr.append(0x15);
arr.append(0x5e);
arr.append(0x3a);
arr.append(0x49);
arr.append(0xa3);
arr.append(0x39);
arr.append(0x14);
arr.append(0x58);
arr.append(0x17);
arr.append(0x29);
arr.append(0xb1);
arr.append(0x1f);
arr.append(0xe0);
arr.append(0x60);
arr.append(0x2b);
arr.append(0xba);
arr.append(0xc5);
arr.append(0x4b);
arr.append(0x49);
arr.append(0x5c);
arr.append(0x1a);
arr.append(0x96);
arr.append(0xb5);
arr.append(0xff);
arr.append(0x16);
arr.append(0x15);
arr.append(0x2a);
arr.append(0x65);
arr.append(0x96);
arr.append(0x17);
arr.append(0xc7);
arr.append(0xb7);
arr.append(0x9d);
arr.append(0x82);
arr.append(0x45);
arr.append(0xc7);
arr.append(0xd4);
arr.append(0xab);
arr.append(0xa6);
arr.append(0x5b);
arr.append(0x5d);
arr.append(0x5c);
arr.append(0xcc);
arr.append(0x94);
arr.append(0x1e);
arr.append(0x24);
arr.append(0xc0);
arr.append(0x9d);
arr.append(0xea);
arr.append(0x26);
arr.append(0x30);
arr.append(0xd1);
arr.append(0xac);
arr.append(0xda);
arr.append(0x68);
arr.append(0x11);
arr.append(0xb2);
arr.append(0x1b);
arr.append(0x78);
arr.append(0xac);
arr.append(0x89);
arr.append(0xb6);
arr.append(0x62);
arr.append(0x58);
arr.append(0x1c);
arr.append(0xba);
arr.append(0x46);
arr.append(0x58);
arr.append(0x62);
arr.append(0x57);
arr.append(0xc7);
arr.append(0x34);
arr.append(0x80);
arr.append(0x22);
arr.append(0x63);
arr.append(0x29);
arr.append(0x48);
arr.append(0xcb);
arr.append(0x1c);
arr.append(0xc8);
arr.append(0xa1);
arr.append(0x2c);
arr.append(0x8c);
arr.append(0x67);
arr.append(0x12);
arr.append(0x17);
arr.append(0x6e);
arr.append(0x61);
arr.append(0x54);
arr.append(0xd5);
arr.append(0xb2);
arr.append(0xce);
arr.append(0x9e);
arr.append(0x17);
arr.append(0xb2);
arr.append(0x0e);
arr.append(0xd7);
arr.append(0x70);
arr.append(0xfc);
arr.append(0x95);
arr.append(0x9f);
arr.append(0x03);
arr.append(0xf4);
arr.append(0xa2);
arr.append(0xe2);
arr.append(0x51);
arr.append(0x84);
arr.append(0x5c);
arr.append(0xa1);
arr.append(0x28);
arr.append(0x03);
arr.append(0x1c);
arr.append(0x5f);
arr.append(0x37);
arr.append(0x63);
arr.append(0x1f);
arr.append(0x6e);
arr.append(0x62);
arr.append(0x41);
arr.append(0xfc);
arr.append(0xe1);
arr.append(0x31);
arr.append(0x16);
arr.append(0x40);
arr.append(0x92);
arr.append(0xba);
arr.append(0x67);
arr.append(0x64);
arr.append(0x83);
arr.append(0xc7);
arr.append(0x05);
arr.append(0x08);
arr.append(0x90);
arr.append(0x27);
arr.append(0x96);
arr.append(0x72);
arr.append(0xa9);
arr.append(0xf8);
arr.append(0xa2);
arr.append(0x32);
arr.append(0x9b);
arr.append(0x75);
arr.append(0x2e);
arr.append(0xb9);
arr.append(0xb0);
arr.append(0x9e);
arr.append(0xb7);
arr.append(0xe4);
arr.append(0x74);
arr.append(0x7e);
arr.append(0x13);
arr.append(0x58);
arr.append(0x7e);
arr.append(0xc5);
arr.append(0x4b);
arr.append(0x99);
arr.append(0x58);
arr.append(0x2e);
arr.append(0x2d);
arr.append(0x90);
arr.append(0x5e);
arr.append(0xd2);
arr.append(0xb5);
arr.append(0x5e);
arr.append(0xf7);
arr.append(0xaa);
arr.append(0x52);
arr.append(0xa1);
arr.append(0x85);
arr.append(0xca);
arr.append(0xac);
arr.append(0xf0);
arr.append(0x37);
arr.append(0xd4);
arr.append(0x94);
arr.append(0x6a);
arr.append(0x95);
arr.append(0xc9);
arr.append(0x4f);
arr.append(0x0b);
arr.append(0x80);
arr.append(0x42);
arr.append(0x3f);
arr.append(0xd4);
arr.append(0x88);
arr.append(0x4a);
arr.append(0x83);
arr.append(0x70);
arr.append(0xaf);
arr.append(0x57);
arr.append(0x6f);
arr.append(0x0e);
arr.append(0x4b);
arr.append(0x1b);
arr.append(0x6c);
arr.append(0x85);
arr.append(0x15);
arr.append(0x83);
arr.append(0x31);
arr.append(0x25);
arr.append(0x00);
arr.append(0xb4);
arr.append(0x42);
arr.append(0x06);
arr.append(0x76);
arr.append(0xa2);
arr.append(0x88);
arr.append(0x01);
arr.append(0x9b);
arr.append(0xfd);
arr.append(0x8c);
arr.append(0x2c);
arr.append(0x24);
arr.append(0x11);
arr.append(0x6e);
arr.append(0xec);
arr.append(0x8c);
arr.append(0x5d);
arr.append(0xd5);
arr.append(0x82);
arr.append(0x2d);
arr.append(0xa8);
arr.append(0x4c);
arr.append(0x44);
arr.append(0x6e);
arr.append(0x09);
arr.append(0xc4);
arr.append(0x78);
arr.append(0x87);
arr.append(0xb9);
arr.append(0xc9);
arr.append(0x12);
arr.append(0xb8);
arr.append(0xc1);
arr.append(0x78);
arr.append(0x6f);
arr.append(0x9a);
arr.append(0xd8);
arr.append(0x3e);
arr.append(0x1d);
arr.append(0x36);
arr.append(0x96);
arr.append(0x17);
arr.append(0x50);
arr.append(0x17);
arr.append(0x53);
arr.append(0x98);
arr.append(0x60);
arr.append(0x6b);
arr.append(0x42);
arr.append(0xb2);
arr.append(0xc3);
arr.append(0xa0);
arr.append(0x95);
arr.append(0xd1);
arr.append(0x04);
arr.append(0xbc);
arr.append(0x0a);
arr.append(0xac);
arr.append(0x45);
arr.append(0x5a);
arr.append(0x72);
arr.append(0x55);
arr.append(0x4a);
arr.append(0xa9);
arr.append(0xac);
arr.append(0x1d);
arr.append(0x9a);
arr.append(0xc5);
arr.append(0xa8);
arr.append(0x01);
arr.append(0xab);
arr.append(0x8c);
arr.append(0x67);
arr.append(0x23);
arr.append(0xd5);
arr.append(0x3a);
arr.append(0x25);
arr.append(0x9b);
arr.append(0xa2);
arr.append(0x0c);
arr.append(0x09);
arr.append(0x4b);
arr.append(0xbe);
arr.append(0x30);
arr.append(0x94);
arr.append(0x53);
arr.append(0x4c);
arr.append(0x88);
arr.append(0x81);
arr.append(0x77);
arr.append(0x03);
arr.append(0xa1);
arr.append(0x14);
arr.append(0xf9);
arr.append(0x4b);
arr.append(0x20);
arr.append(0x70);
arr.append(0xc4);
arr.append(0x90);
arr.append(0x1a);
arr.append(0x4b);
arr.append(0xb7);
arr.append(0xd0);
arr.append(0x28);
arr.append(0x58);
arr.append(0xb3);
arr.append(0x1d);
arr.append(0xbc);
arr.append(0x61);
arr.append(0x4a);
arr.append(0xa7);
arr.append(0xc3);
arr.append(0xa1);
arr.append(0x03);
arr.append(0x44);
arr.append(0x95);
arr.append(0x6b);
arr.append(0x94);
arr.append(0x0e);
arr.append(0xb6);
arr.append(0x19);
arr.append(0xad);
arr.append(0xaf);
arr.append(0xd9);
arr.append(0xc2);
arr.append(0x93);
arr.append(0x97);
arr.append(0xbd);
arr.append(0x63);
arr.append(0xc0);
arr.append(0x60);
arr.append(0x0d);
arr.append(0x91);
arr.append(0x0e);
arr.append(0x97);
arr.append(0xa0);
arr.append(0x55);
arr.append(0x45);
arr.append(0x15);
arr.append(0x16);
arr.append(0x32);
arr.append(0x21);
arr.append(0x19);
arr.append(0x4d);
arr.append(0xc2);
arr.append(0xbc);
arr.append(0xec);
arr.append(0xb5);
arr.append(0x78);
arr.append(0x6f);
arr.append(0x08);
arr.append(0x1f);
arr.append(0x79);
arr.append(0x47);
arr.append(0x8a);
arr.append(0xf2);
arr.append(0xf2);
arr.append(0x55);
arr.append(0x90);
arr.append(0x56);
arr.append(0xb6);
arr.append(0x64);
arr.append(0x53);
arr.append(0x78);
arr.append(0x9c);
arr.append(0xe5);
arr.append(0x38);
arr.append(0x77);
arr.append(0xdb);
arr.append(0x74);
arr.append(0x3a);
arr.append(0x3c);
arr.append(0x83);
arr.append(0x78);
arr.append(0xd2);
arr.append(0x0f);
arr.append(0x92);
arr.append(0xb3);
arr.append(0xc4);
arr.append(0x2f);
arr.append(0x46);
arr.append(0xa7);
arr.append(0x12);
arr.append(0xaa);
arr.append(0x92);
arr.append(0xeb);
arr.append(0x76);
arr.append(0x5d);
arr.append(0xe9);
arr.append(0x33);
arr.append(0xbd);
arr.append(0x0a);
arr.append(0x53);
arr.append(0xa5);
arr.append(0x31);
arr.append(0xf5);
arr.append(0x52);
arr.append(0x21);
arr.append(0x8c);
arr.append(0x5d);
arr.append(0xa3);
arr.append(0xb5);
arr.append(0x77);
arr.append(0x75);
arr.append(0x88);
arr.append(0x1b);
arr.append(0xbd);
arr.append(0x82);
arr.append(0x40);
arr.append(0xea);
arr.append(0xbc);
arr.append(0x18);
arr.append(0x81);
arr.append(0xec);
arr.append(0x35);
arr.append(0x19);
arr.append(0xca);
arr.append(0x80);
arr.append(0x24);
arr.append(0x3a);
arr.append(0x8f);
arr.append(0xc4);
arr.append(0xcc);
arr.append(0xaa);
arr.append(0x71);
arr.append(0x08);
arr.append(0x09);
arr.append(0x9a);
arr.append(0x95);
arr.append(0x97);
arr.append(0x26);
arr.append(0xb7);
arr.append(0x2a);
arr.append(0xee);
arr.append(0x52);
arr.append(0x27);
arr.append(0x80);
arr.append(0x64);
arr.append(0x9c);
arr.append(0x91);
arr.append(0xf4);
arr.append(0xa8);
arr.append(0x2a);
arr.append(0xab);
arr.append(0x20);
arr.append(0x45);
arr.append(0x83);
arr.append(0x4b);
arr.append(0x2f);
arr.append(0x64);
arr.append(0x03);
arr.append(0x9e);
arr.append(0x54);
arr.append(0x2c);
arr.append(0x37);
arr.append(0xb7);
arr.append(0xb8);
arr.append(0x1f);
arr.append(0x60);
arr.append(0x2b);
arr.append(0xe2);
arr.append(0xf0);
arr.append(0x68);
arr.append(0x8c);
arr.append(0xb5);
arr.append(0x25);
arr.append(0x0f);
arr.append(0x29);
arr.append(0x55);
arr.append(0x91);
arr.append(0xc9);
arr.append(0x99);
arr.append(0x39);
arr.append(0x04);
arr.append(0x99);
arr.append(0x82);
arr.append(0x21);
arr.append(0x40);
arr.append(0xff);
arr.append(0x43);
arr.append(0x25);
arr.append(0x01);
arr.append(0x18);
arr.append(0x60);
arr.append(0xfa);
arr.append(0x75);
arr.append(0x3b);
arr.append(0x81);
arr.append(0xe8);
arr.append(0x5b);
arr.append(0xf2);
arr.append(0x9c);
arr.append(0x86);
arr.append(0x6c);
arr.append(0x36);
arr.append(0x1d);
arr.append(0x91);
arr.append(0x03);
arr.append(0x41);
arr.append(0xf2);
arr.append(0x96);
arr.append(0xc6);
arr.append(0x4b);
arr.append(0x46);
arr.append(0xc2);
arr.append(0xa2);
arr.append(0xe3);
arr.append(0x0b);
arr.append(0x15);
arr.append(0x35);
arr.append(0xa5);
arr.append(0xc0);
arr.append(0x60);
arr.append(0x25);
arr.append(0x93);
arr.append(0x41);
arr.append(0x5d);
arr.append(0x15);
arr.append(0x6b);
arr.append(0x43);
arr.append(0x03);
arr.append(0x6b);
arr.append(0x21);
arr.append(0xb1);
arr.append(0x4b);
arr.append(0xa3);
arr.append(0xa0);
arr.append(0xc7);
arr.append(0x2e);
arr.append(0x84);
arr.append(0x8c);
arr.append(0x5e);
arr.append(0xe0);
arr.append(0x34);
arr.append(0x66);
arr.append(0xca);
arr.append(0xb9);
arr.append(0x77);
arr.append(0x21);
arr.append(0xd8);
arr.append(0x3d);
arr.append(0xef);
arr.append(0xda);
arr.append(0xb1);
arr.append(0xf6);
arr.append(0x70);
arr.append(0x89);
arr.append(0x71);
arr.append(0xd3);
arr.append(0xc4);
arr.append(0x58);
arr.append(0x14);
arr.append(0x41);
arr.append(0xce);
arr.append(0x39);
arr.append(0x7f);
arr.append(0xe7);
arr.append(0xbb);
arr.append(0xc7);
arr.append(0xa0);
arr.append(0x8a);
arr.append(0xcf);
arr.append(0xdc);
arr.append(0xc4);
arr.append(0x17);
arr.append(0xc5);
arr.append(0xf5);
arr.append(0x6d);
arr.append(0xfd);
arr.append(0xd4);
arr.append(0x42);
arr.append(0x3e);
arr.append(0x30);
arr.append(0x6f);
arr.append(0x1f);
arr.append(0x69);
arr.append(0xa8);
arr.append(0x6e);
arr.append(0x5b);
arr.append(0x5b);
arr.append(0xa3);
arr.append(0xe0);
arr.append(0x31);
arr.append(0xbf);
arr.append(0x92);
arr.append(0xa1);
arr.append(0x67);
arr.append(0x02);
arr.append(0x86);
arr.append(0x1a);
arr.append(0x51);
arr.append(0xa2);
arr.append(0xfe);
arr.append(0xb9);
arr.append(0x74);
arr.append(0xf4);
arr.append(0x84);
arr.append(0x48);
arr.append(0x12);
arr.append(0xb8);
arr.append(0x30);
arr.append(0x2d);
arr.append(0xc8);
arr.append(0x30);
arr.append(0x26);
arr.append(0xea);
arr.append(0x56);
arr.append(0x6a);
arr.append(0x1c);
arr.append(0x11);
arr.append(0x0c);
arr.append(0xd2);
arr.append(0xc4);
arr.append(0xaf);
arr.append(0x48);
arr.append(0xbe);
arr.append(0xdd);
arr.append(0x74);
arr.append(0x2f);
arr.append(0x1c);
arr.append(0x02);
arr.append(0xcd);
arr.append(0x3f);
arr.append(0xca);
arr.append(0x0d);
arr.append(0xfb);
arr.append(0x83);
arr.append(0x31);
arr.append(0xf4);
arr.append(0x14);
arr.append(0xb1);
arr.append(0x15);
arr.append(0x84);
arr.append(0x9b);
arr.append(0xcb);
arr.append(0x05);
arr.append(0xa2);
arr.append(0xdf);
arr.append(0x61);
arr.append(0x5b);
arr.append(0x49);
arr.append(0xa9);
arr.append(0x0e);
arr.append(0xc0);
arr.append(0xa4);
arr.append(0x33);
arr.append(0xe8);
arr.append(0x78);
arr.append(0x7e);
arr.append(0x18);
arr.append(0x51);
arr.append(0x42);
arr.append(0xaa);
arr.append(0x40);
arr.append(0x60);
arr.append(0x03);
arr.append(0xd3);
arr.append(0x54);
arr.append(0x09);
arr.append(0xec);
arr.append(0x84);
arr.append(0x2a);
arr.append(0xf3);
arr.append(0x04);
arr.append(0x45);
arr.append(0x9b);
arr.append(0x40);
arr.append(0xff);
arr.append(0xfa);
arr.append(0x0d);
arr.append(0x84);
arr.append(0x34);
arr.append(0x01);
arr.append(0x56);
arr.append(0xb5);
arr.append(0x76);
arr.append(0x7e);
arr.append(0xe0);
arr.append(0xca);
arr.append(0x6e);
arr.append(0x28);
arr.append(0x50);
arr.append(0xec);
arr.append(0x8c);
arr.append(0xca);
arr.append(0xed);
arr.append(0x31);
arr.append(0x7e);
arr.append(0xcf);
arr.append(0x71);
arr.append(0x1e);
arr.append(0x27);
arr.append(0xd5);
arr.append(0x9b);
arr.append(0x3c);
arr.append(0x98);
arr.append(0x55);
arr.append(0x55);
arr.append(0xe4);
arr.append(0x4c);
arr.append(0x93);
arr.append(0x59);
arr.append(0x88);
arr.append(0x09);
arr.append(0x29);
arr.append(0x81);
arr.append(0xda);
arr.append(0x95);
arr.append(0xb4);
arr.append(0x25);
arr.append(0x73);
arr.append(0xb9);
arr.append(0xbc);
arr.append(0x38);
arr.append(0x12);
arr.append(0x2e);
arr.append(0xd4);
arr.append(0x4a);
arr.append(0x80);
arr.append(0x05);
arr.append(0x10);
arr.append(0x75);
arr.append(0x64);
arr.append(0x29);
arr.append(0xf1);
arr.append(0xb0);
arr.append(0x64);
arr.append(0x06);
arr.append(0x85);
arr.append(0xb0);
arr.append(0x48);
arr.append(0x59);
arr.append(0x78);
arr.append(0x42);
arr.append(0xcb);
arr.append(0xb9);
arr.append(0x3d);
arr.append(0xc6);
arr.append(0xb2);
arr.append(0xf9);
arr.append(0x14);
arr.append(0x4d);
arr.append(0xcf);
arr.append(0x41);
arr.append(0xa6);
arr.append(0x24);
arr.append(0xeb);
arr.append(0x90);
arr.append(0x20);
arr.append(0x2a);
arr.append(0x30);
arr.append(0x38);
arr.append(0x14);
arr.append(0x10);
arr.append(0xf4);
arr.append(0xcb);
arr.append(0xaa);
arr.append(0xcc);
arr.append(0xf6);
arr.append(0x1e);
arr.append(0x48);
arr.append(0x55);
arr.append(0x24);
arr.append(0xee);
arr.append(0x59);
arr.append(0x3a);
arr.append(0xd7);
arr.append(0xd5);
arr.append(0x97);
arr.append(0x4f);
arr.append(0x77);
arr.append(0x4a);
arr.append(0xe5);
arr.append(0x9b);
arr.append(0xa2);
arr.append(0x0c);
arr.append(0x7c);
arr.append(0x6e);
arr.append(0xb0);
arr.append(0x00);
arr.append(0x9b);
arr.append(0x39);
arr.append(0x96);
arr.append(0x2d);
arr.append(0xc0);
arr.append(0xf8);
arr.append(0x28);
arr.append(0xe8);
arr.append(0x53);
arr.append(0x8e);
arr.append(0xf5);
arr.append(0x83);
arr.append(0xb4);
arr.append(0xdc);
arr.append(0x40);
arr.append(0xc0);
arr.append(0x95);
arr.append(0x55);
arr.append(0xa3);
arr.append(0x96);
arr.append(0x8a);
arr.append(0xb2);
arr.append(0x57);
arr.append(0xd4);
arr.append(0x07);
arr.append(0xcc);
arr.append(0x4a);
arr.append(0xb8);
arr.append(0x01);
arr.append(0xc7);
arr.append(0x52);
arr.append(0x1b);
arr.append(0xf7);
arr.append(0x56);
arr.append(0x4b);
arr.append(0x16);
arr.append(0x3f);
arr.append(0xab);
arr.append(0x92);
arr.append(0x7b);
arr.append(0x47);
arr.append(0x03);
arr.append(0x22);
arr.append(0x5b);
arr.append(0x1b);
arr.append(0x49);
arr.append(0x7a);
arr.append(0x09);
arr.append(0x6b);
arr.append(0x4e);
arr.append(0xb9);
arr.append(0x84);
arr.append(0xdf);
arr.append(0xd3);
arr.append(0x51);
arr.append(0x2e);
arr.append(0xcb);
arr.append(0x2e);
arr.append(0xae);
arr.append(0xa4);
arr.append(0x6f);
arr.append(0xb5);
arr.append(0x3c);
arr.append(0x3b);
arr.append(0x38);
arr.append(0x40);
arr.append(0xaf);
arr.append(0x3c);
arr.append(0x4a);
arr.append(0xaf);
arr.append(0x78);
arr.append(0xb1);
arr.append(0x46);
arr.append(0x1e);
arr.append(0x05);
arr.append(0x64);
arr.append(0x29);
arr.append(0x0c);
arr.append(0x0a);
arr.append(0x8f);
arr.append(0x93);
arr.append(0xa1);
arr.append(0xe1);
arr.append(0xb1);
arr.append(0xc5);
arr.append(0x0c);
arr.append(0xdb);
arr.append(0x5f);
arr.append(0xd5);
arr.append(0x69);
arr.append(0x70);
arr.append(0x04);
arr.append(0x61);
arr.append(0x03);
arr.append(0x60);
arr.append(0x0a);
arr.append(0x1c);
arr.append(0xc1);
arr.append(0x09);
arr.append(0xb5);
arr.append(0x8c);
arr.append(0xc3);
arr.append(0x2e);
arr.append(0x52);
arr.append(0xc7);
arr.append(0xa1);
arr.append(0x68);
arr.append(0xe1);
arr.append(0x71);
arr.append(0x07);
arr.append(0xa9);
arr.append(0xc7);
arr.append(0x7c);
arr.append(0xc4);
arr.append(0x48);
arr.append(0xb0);
arr.append(0x1a);
arr.append(0x5a);
arr.append(0x7d);
arr.append(0xc9);
arr.append(0x06);
arr.append(0x03);
arr.append(0xd7);
arr.append(0x08);
arr.append(0xc6);
arr.append(0xd7);
arr.append(0x17);
arr.append(0xc3);
arr.append(0x16);
arr.append(0x5e);
arr.append(0x77);
arr.append(0x2c);
arr.append(0x74);
arr.append(0x76);
arr.append(0x38);
arr.append(0x21);
arr.append(0xf3);
arr.append(0x59);
arr.append(0x34);
arr.append(0xa6);
arr.append(0x13);
arr.append(0xb0);
arr.append(0x29);
arr.append(0x72);
arr.append(0x46);
arr.append(0x37);
arr.append(0x00);
arr.append(0x20);
arr.append(0xeb);
arr.append(0x11);
arr.append(0x36);
arr.append(0x45);
arr.append(0xd7);
arr.append(0x55);
arr.append(0x93);
arr.append(0xc5);
arr.append(0x13);
arr.append(0x00);
arr.append(0x72);
arr.append(0xac);
arr.append(0xc0);
arr.append(0x48);
arr.append(0x31);
arr.append(0x18);
arr.append(0x06);
arr.append(0x7b);
arr.append(0xfb);
arr.append(0x80);
arr.append(0x93);
arr.append(0xb5);
arr.append(0x18);
arr.append(0x06);
arr.append(0x63);
arr.append(0x18);
arr.append(0xc3);
arr.append(0xf9);
arr.append(0x36);
arr.append(0xbd);
arr.append(0x29);
arr.append(0xe8);
arr.append(0xac);
arr.append(0xba);
arr.append(0x83);
arr.append(0x18);
arr.append(0xc6);
arr.append(0x8a);
arr.append(0x3c);
arr.append(0x47);
arr.append(0xa1);
arr.append(0xbe);
arr.append(0x71);
arr.append(0x3c);
arr.append(0x5b);
arr.append(0x5a);
arr.append(0x3c);
arr.append(0x3d);
arr.append(0xf8);
arr.append(0x25);
arr.append(0x14);
arr.append(0xa5);
arr.append(0xdb);
arr.append(0x7f);
arr.append(0x70);
arr.append(0x15);
arr.append(0x0b);
arr.append(0x44);
arr.append(0xb3);
arr.append(0x92);
arr.append(0x7a);
arr.append(0xa5);
arr.append(0xc7);
arr.append(0xb5);
arr.append(0x50);
arr.append(0x7f);
arr.append(0x8e);
arr.append(0x69);
arr.append(0x3e);
arr.append(0xed);
arr.append(0x04);
arr.append(0x36);
arr.append(0x93);
arr.append(0x40);
arr.append(0x14);
arr.append(0x1b);
arr.append(0x1c);
arr.append(0x75);
arr.append(0x06);
arr.append(0x9b);
arr.append(0x5f);
arr.append(0x3a);
arr.append(0xbb);
arr.append(0x01);
arr.append(0x68);
arr.append(0x33);
arr.append(0xb9);
arr.append(0x36);
arr.append(0xa1);
arr.append(0xad);
arr.append(0xf7);
arr.append(0xcc);
arr.append(0xbe);
arr.append(0xdf);
arr.append(0xe7);
arr.append(0x75);
arr.append(0xf0);
arr.append(0x49);
arr.append(0x4a);
arr.append(0xcb);
arr.append(0x64);
arr.append(0x04);
arr.append(0x30);
arr.append(0x7a);
arr.append(0x78);
arr.append(0x7e);
arr.append(0x19);
arr.append(0xb2);
arr.append(0xfc);
arr.append(0xba);
arr.append(0x2a);
arr.append(0xa8);
arr.append(0xb3);
arr.append(0xcf);
arr.append(0x94);
arr.append(0xea);
arr.append(0x6a);
arr.append(0xa4);
arr.append(0x30);
arr.append(0x3c);
arr.append(0xfe);
arr.append(0x48);
arr.append(0x21);
arr.append(0xfc);
arr.append(0x9a);
arr.append(0x62);
arr.append(0xee);
arr.append(0x5a);
arr.append(0xcb);
arr.append(0xca);
arr.append(0xa4);
arr.append(0x25);
arr.append(0x56);
arr.append(0xd0);
arr.append(0x46);
arr.append(0xe4);
arr.append(0xab);
arr.append(0x2e);
arr.append(0x65);
arr.append(0xe5);
arr.append(0x7e);
arr.append(0x4d);
arr.append(0xcc);
arr.append(0x87);
arr.append(0x42);
arr.append(0x76);
arr.append(0xc8);
arr.append(0x9f);
arr.append(0x98);
arr.append(0x0a);
arr.append(0xc1);
arr.append(0xa5);
arr.append(0x2f);
arr.append(0xde);
arr.append(0x54);
arr.append(0x90);
arr.append(0x64);
arr.append(0xb7);
arr.append(0xab);
arr.append(0xea);
arr.append(0xb3);
arr.append(0x0e);
arr.append(0x81);
arr.append(0xd5);
arr.append(0x40);
arr.append(0xaf);
arr.append(0x39);
arr.append(0x3a);
arr.append(0x51);
arr.append(0xbb);
arr.append(0x91);
arr.append(0x7d);
arr.append(0x87);
arr.append(0x09);
arr.append(0x8a);
arr.append(0xe2);
arr.append(0x38);
arr.append(0x94);
arr.append(0x60);
arr.append(0x33);
arr.append(0xf3);
arr.append(0xbb);
arr.append(0x31);
arr.append(0xd8);
arr.append(0x2b);
arr.append(0x41);
arr.append(0x8e);
arr.append(0xa8);
arr.append(0x63);
arr.append(0xb9);
arr.append(0xfc);
arr.append(0x20);
arr.append(0x08);
arr.append(0x61);
arr.append(0x3e);
arr.append(0x47);
arr.append(0x51);
arr.append(0xae);
arr.append(0xd8);
arr.append(0xc9);
arr.append(0xaa);
arr.append(0xa4);
arr.append(0x18);
arr.append(0x40);
arr.append(0x57);
arr.append(0x91);
arr.append(0x16);
arr.append(0xb5);
arr.append(0x71);
arr.append(0xad);
arr.append(0x6d);
arr.append(0xf0);
arr.append(0xcf);
arr.append(0xc1);
arr.append(0x18);
arr.append(0x5b);
arr.append(0x17);
arr.append(0x61);
arr.append(0x20);
arr.append(0x08);
arr.append(0x90);
arr.append(0x0d);
arr.append(0x10);
arr.append(0x4a);
arr.append(0x83);
arr.append(0xb6);
arr.append(0x7a);
arr.append(0x36);
arr.append(0xac);
arr.append(0xd8);
arr.append(0xc7);
arr.append(0x83);
arr.append(0xc5);
arr.append(0xb1);
arr.append(0xc2);
arr.append(0x10);
arr.append(0x26);
arr.append(0xf0);
arr.append(0x3b);
arr.append(0x4a);
arr.append(0xb5);
arr.append(0x7b);
arr.append(0x23);
arr.append(0xbd);
arr.append(0x16);
arr.append(0x0a);
arr.append(0x10);
arr.append(0x39);
arr.append(0x20);
arr.append(0x72);
arr.append(0x9b);
arr.append(0x13);
arr.append(0x33);
arr.append(0x43);
arr.append(0x95);
arr.append(0xa5);
arr.append(0x00);
arr.append(0xa2);
arr.append(0xa5);
arr.append(0xa1);
arr.append(0x26);
arr.append(0xbd);
arr.append(0x66);
arr.append(0xc7);
arr.append(0x5c);
arr.append(0x5c);
arr.append(0x2a);
arr.append(0x04);
arr.append(0x6a);
arr.append(0x97);
arr.append(0xeb);
arr.append(0xb8);
arr.append(0x56);
arr.append(0x5c);
arr.append(0xbb);
arr.append(0x65);
arr.append(0xfc);
arr.append(0x6a);
arr.append(0x3e);
arr.append(0x1c);
arr.append(0x11);
arr.append(0xa9);
arr.append(0x90);
arr.append(0xe2);
arr.append(0x0a);
arr.append(0x9c);
arr.append(0x88);
arr.append(0xc9);
arr.append(0x6a);
arr.append(0xa3);
arr.append(0x92);
arr.append(0x49);
arr.append(0x61);
arr.append(0x15);
arr.append(0x9e);
arr.append(0xf2);
arr.append(0x20);
arr.append(0x82);
arr.append(0xe4);
arr.append(0x5c);
arr.append(0x86);
arr.append(0xa9);
arr.append(0xbb);
arr.append(0x80);
arr.append(0xd7);
arr.append(0xb8);
arr.append(0x93);
arr.append(0x6c);
arr.append(0xc9);
arr.append(0xee);
arr.append(0xc5);
arr.append(0x93);
arr.append(0xe5);
arr.append(0xe4);
arr.append(0x52);
arr.append(0x3d);
arr.append(0xdc);
arr.append(0x54);
arr.append(0x38);
arr.append(0xc8);
arr.append(0x19);
arr.append(0x55);
arr.append(0x13);
arr.append(0x28);
arr.append(0x55);
arr.append(0x79);
arr.append(0x82);
arr.append(0xcf);
arr.append(0x67);
arr.append(0x84);
arr.append(0xb1);
arr.append(0x72);
arr.append(0x0e);
arr.append(0x4e);
arr.append(0xd5);
arr.append(0xac);
arr.append(0x0a);
arr.append(0xdd);
arr.append(0x45);
arr.append(0x7f);
arr.append(0x57);
arr.append(0x3a);
arr.append(0x04);
arr.append(0x14);
arr.append(0x65);
arr.append(0xbc);
arr.append(0xbd);
arr.append(0x7c);
arr.append(0xa4);
arr.append(0xe1);
arr.append(0xd7);
arr.append(0xd5);
arr.append(0x3e);
arr.append(0xaa);
arr.append(0xde);
arr.append(0xda);
arr.append(0x51);
arr.append(0x19);
arr.append(0x62);
arr.append(0xa3);
arr.append(0x6e);
arr.append(0xb0);
arr.append(0xa9);
arr.append(0xce);
arr.append(0x0e);
arr.append(0xcb);
arr.append(0xcf);
arr.append(0xbb);
arr.append(0xbd);
arr.append(0x9a);
arr.append(0x26);
arr.append(0x36);
arr.append(0xe2);
arr.append(0x3a);
arr.append(0x0c);
arr.append(0x1c);
arr.append(0x22);
arr.append(0x68);
arr.append(0x39);
arr.append(0x43);
arr.append(0x63);
arr.append(0x9c);
arr.append(0xd4);
arr.append(0x4a);
arr.append(0xf0);
arr.append(0x51);
arr.append(0xff);
arr.append(0x23);
arr.append(0x5b);
arr.append(0x8d);
arr.append(0xfb);
arr.append(0xb7);
arr.append(0x3a);
arr.append(0x4d);
arr.append(0xc6);
arr.append(0xf5);
arr.append(0x78);
arr.append(0x5a);
arr.append(0x6f);
arr.append(0x2b);
arr.append(0x42);
arr.append(0xe8);
arr.append(0x43);
arr.append(0x22);
arr.append(0x8b);
arr.append(0xe5);
arr.append(0x3e);
arr.append(0xb7);
arr.append(0x68);
arr.append(0xd6);
arr.append(0x4c);
arr.append(0x6f);
arr.append(0x9d);
arr.append(0x43);
arr.append(0x55);
arr.append(0xae);
arr.append(0x95);
arr.append(0xf0);
arr.append(0x83);
arr.append(0xe5);
arr.append(0x1e);
arr.append(0xd5);
arr.append(0x7c);
arr.append(0x43);
arr.append(0x73);
arr.append(0x10);
arr
}